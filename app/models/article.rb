class Article < ApplicationRecord
  def self.news_title(search)
    ...
    #page = agent.get"https://search.naver.com/search.naver?sm=tab_hty.top&where=news&ie=utf8&query=1&start=1"
    amount = page.search("div.title_desc.all_my span").text
    amount = amount.split("건").last
    amount = amount.split(" / ")
    amount = amount[1].delete(",").to_i
    ### 값이 너무 커지는 경우를 제어하기 위해
    if amount > 100
      amount = 100
    end
    arrays = Array.new(amount/10){ |i| ((i+1)*10).to_s }
